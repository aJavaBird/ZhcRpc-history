...
public class ZookeeperUtil {
     ...
     * @throws InterruptedException 
     * @throws KeeperException */
    public static void createNodeIfNotExists(String path, ZooKeeper zk) throws KeeperException, InterruptedException {
        createNodeIfNotExists(path, CreateMode.PERSISTENT, zk);
    }

    /** 节点不存在则创建，返回true表示新创建且成功，返回false表示参数有误或节点已存在 */
    public static Boolean createNodeIfNotExists(String path, CreateMode mode, ZooKeeper zk)
            throws KeeperException, InterruptedException {
        if (path == null || !path.startsWith("/") || zk == null) {
            logger.error("参数不合法！");
            return Boolean.FALSE;
        }
        String pathArr[] = path.split("/"); // pathArr[0] == ""
        if (pathArr.length > 1) {
            StringBuilder workPath = new StringBuilder("");
            for (int i = 1; i < pathArr.length; i++) {
                workPath.append("/").append(pathArr[i]);
                Stat stat = zk.exists(workPath.toString(), false);
                if (stat == null) {
                    zk.create(workPath.toString(), null, Ids.OPEN_ACL_UNSAFE, mode);
                    return Boolean.TRUE;
                }
            }
        }
